(function(w, d, u) {
	...
	var ajax = function(options) {
		...
		if (options.type == "POST") {
			...
			xhr.open("GET", options.url + "?" + formatParams(options.data),
					true);
			xhr.send(null);
		}
	}
	...
	Layer.prototype = {
		...
		rend : function() {
			var html = this.template.replace('{title}', this.title).replace(
					'{content}', this.content).replace('{confirmText}',
					this.confirmText);
	...
	Loading.prototype = {
		...
		rend : function(data) {
			var html = data ? this.template.replace('{class}', 'v-load-result')
					.replace('{message}', data.message) : this.template;
			this.body.innerHTML = html;
		},
		show : function() {
			this.rend();
