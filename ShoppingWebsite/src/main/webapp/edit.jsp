					...
					<div class="fmipt" id="urlUpload" style="display: block;">
						<c:choose>
							<c:when test="${content.image != '1'}">
								<input id="url" class="u-ipt ipt" name="image"
									placeholder="图片地址" value="${content.image}">
							</c:when>
							<c:otherwise>
								<input id="url" class="u-ipt ipt" name="image"
									placeholder="图片地址" value="1">
							</c:otherwise>
						</c:choose>
					</div>
					<div class="fmipt" id="fileUpload" style="display: none;">
						<c:if test="${content.file != ''}">
							...
							<button id="change" class="u-btn u-btn-primary" onclick="del()">更改</button>
						</c:if>
